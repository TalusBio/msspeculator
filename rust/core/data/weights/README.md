...
**The authoritative provenance is inside each file**, not in this note. Every artifact written by
...
`every_bundled_artifact_matches_its_recorded_digest` fails — which is the point: weights cannot be
swapped under a name without saying so.

### Size

`MAX_BUNDLED_BYTES` (4 MiB) caps what may be embedded, enforced by the same test. Weights larger
than that do not belong in version control; they belong behind a runtime fetch into a cache, which
leaves clean-clone builds working. The `base` preset would still fit; a substantially larger model
would not.
